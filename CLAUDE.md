...
- **Suporte a múltiplos nomes de dispositivo BT** — implementado em `feature/bt-multi-name`. `isBtDeviceAllowed(name)` em `UHFReaderService.companion` aceita `"Winnix_BT"` (legado) ou `"SPACEVIS_RFID_XXXX"` (prefixo maiúsculo + 4 chars hex do MAC, comprimento fixo 18). Todos os pontos que antes comparavam com a constante usam essa função. `refreshDeviceList` exibe todos os pareados aceitos; reconexão usa `activeDeviceName`; `ACTION_ACL_CONNECTED` verifica `getActiveDeviceName() == connectedName` para não retomar no dispositivo errado.
