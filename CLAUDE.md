...
Current version: **1.2.5** (versionCode 10). O flavor `hml` acrescenta `-hml` ao `versionName` (ex.: `1.2.5-hml`); o `versionCode` é o mesmo para os dois. Always commit the version bump together with the change that triggered it, not as a separate commit afterwards.
